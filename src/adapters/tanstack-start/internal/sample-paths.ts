import type {
  ...
  TanStackStartRouterRoutesByPath,
} from './types.js';

import { createTanStackStartRouteTemplates } from './routes.js';
import { prepareTanStackStartSitemapPaths } from './sitemap.js';
 ...
 */
export function getSamplePaths<TRouter extends TanStackStartRouterRoutesByPath>({
  getCanonicalPath = identityPath,
  sitemapConfig,
}: GetSamplePathsOptions<TRouter>): string[] {
  const paths = prepareTanStackStartSitemapPaths(sitemapConfig).map(({ path }) =>
 ...
 */
function createSampleRouteMatchers(templates: TanStackStartRouteTemplate[]): SampleRouteMatcher[] {
  return templates
    .map((template) => ({
 ...
 */
function routeTemplateToRegex(template: TanStackStartRouteTemplate): RegExp {
 ...
 */
function getRouteTemplateSpecificityScore(template: TanStackStartRouteTemplate): number {
